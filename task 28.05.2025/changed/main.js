...
$(document).ready(function() {
    ...
    //   form-check-input pop
    jQuery('.form-check.dropdown .form-check-input.with-pop').click(function(e){
        jQuery('.form-check-input.with-pop').not(this).parents('.form-check.dropdown').find('.dropdown-menu').removeClass('show');
        //  jQuery('.form-check-input.with-pop').not(this).parents('.form-check.dropdown').find('.form-check-input.with-pop').prop('checked', false);
        jQuery(this).parents('.form-check').find('.dropdown-menu').addClass('show');
        if($(this).is(':checked'))
        {
        // jQuery(this).parents('.form-check').find('.dropdown-menu').addClass('show');
        }else
        {
            // jQuery(this).parents('.form-check').find('.dropdown-menu').removeClass('show');
        }
        e.stopPropagation();
    });
    jQuery('.form-check.dropdown .close-this-popup .trigger').click(function(e){
        jQuery(this).parents('.form-check').find('.dropdown-menu').removeClass('show');
        //  jQuery(this).parents('.form-check').find('.form-check-input.with-pop').prop('checked', false);
         e.stopPropagation();
    });
